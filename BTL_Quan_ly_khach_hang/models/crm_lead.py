...
class CrmLead(models.Model):
    ...
    @api.depends('interaction_ids', 'quality_score', 'budget', 'create_date')
        ...
        try:
            ...
            for lead in self:
                ...
                if scoring_service.is_model_loaded():
                    ...
                    lead.ai_score = result.get('conversion_probability', 0)
                    lead.ai_will_convert = result.get('will_convert', False)
                    lead.ai_confidence = result.get('confidence', 0) * 100
                    ...
                    # Model not loaded - use default scoring
                    lead.ai_score = lead.quality_score or 50
                    lead.ai_will_convert = (lead.quality_score or 50) >= 60
                    lead.ai_confidence = 0
                    lead.ai_risk_level = 'medium'
                    lead.ai_last_update = datetime.now()
                    
        except ImportError as e:
            _logger.warning(f"Lead Scoring Service not available: {str(e)}")
            for lead in self:
                lead.ai_score = lead.quality_score or 50
                lead.ai_will_convert = (lead.quality_score or 50) >= 60
                lead.ai_confidence = 0
